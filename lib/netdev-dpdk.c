...
dpdk_mp_create(struct netdev_dpdk *dev, int mtu, bool per_port_mp)
{
    ...
    do {
        ...
        VLOG_INFO("Port %s: Requesting a mempool of %u mbufs of size %u "
                  "on socket %d for %d Rx and %d Tx queues, "
                  "cache line size of %u, mbuf_size=%u private=%u",
                  netdev_name, n_mbufs, mbuf_size, socket_id,
                  dev->requested_n_rxq, dev->requested_n_txq,
                  RTE_CACHE_LINE_SIZE, mbuf_size, mbuf_priv_data_len);
                              ...
                              int pkt_cnt)
{
    ...
    for (i = 0; i < pkt_cnt; i++) {
        ...
        if (dev->type != DPDK_DEV_VHOST && pkt->ol_flags & PKT_TX_TCP_SEG) {
            /* VM doesn't require preparation, only NIC does. */
...
netdev_dpdk_vhost_client_reconfigure(struct netdev *netdev)
{
    ...
    if (!(dev->vhost_driver_flags & RTE_VHOST_USER_CLIENT) && dev->vhost_id) {
        ...
        /* Enable large linear buffers in case TSO gets enabled */
